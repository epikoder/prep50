  ...
  const onSelectForDelete = (
    ...
    if (checked) {
      ...
      deletion_list.value.splice(
        deletion_list.value.findIndex((e: T) => qx.topic_id == e.topic_id),
      );
    }
  ...
  return (
    <div class={"space-y-3"}>
        ...
        render={(topic, index) => (
          <div class={"w-full flex space-x-2"}>
            <div class={"space-y-1"}>
              ...
              <input
                type={"checkbox"}
                onChange={(ev) => onSelectForDelete(ev, topic)}
              />
            </div>
            <a
              href={`/publish/${id}/${subject_id}/${topic.topic_id}`}
              class={"w-full"}
            >
              <div
      ...
      <LinkButton
        ...
        url={`/publish/update/${id}/${subject_id}`}
      />
    </div>
  );
}
